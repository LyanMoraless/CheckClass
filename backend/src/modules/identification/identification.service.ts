...
export class IdentificationService {
  ...
  async processRawEvent(rawEventId: string): Promise<void> {
    ...
    if (payload.roomId) {
      classSessionId = await this.resolveClassSession(manager, tenantId, payload.roomId, payload.capturedAt);
  ...
  // RULE-INST-07: class_session.roomId is persisted NULL on purpose for
  // sessions generated by the recurring grade (SessionGenerationService/
  // ScheduleRegenerationService) — it means "inherit class_group.roomId",
  // not "no room". Matching on session.roomId alone would silently drop
  // every device-originated check-in for such a session. Resolves against
  // the EFFECTIVE room instead — same COALESCE(cs.room_id, cg.room_id)
  // primitive as ScheduleConflictDetectionService and the same
  // session.roomId ?? classGroup.roomId fallback as
  // ClassSessionService.editSession, not a third implementation of it.
  private async resolveClassSession(
    manager: EntityManager,
    tenantId: string,
    roomId: string,
    capturedAt: string,
    ...
    const rows: Array<{ id: string }> = await manager.query(
      ...
      WHERE cs.tenant_id = $1
        AND COALESCE(cs.room_id, cg.room_id) = $2
        AND cs.scheduled_start <= $3::timestamptz
        AND cs.scheduled_end >= $3::timestamptz
      LIMIT 1
      `,
      [tenantId, roomId, capturedDate.toISOString()],
    );
